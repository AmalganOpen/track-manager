    ...
    def _make_request_with_retry(self, url: str) -> Optional[dict]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    print(
                        f"⚠️ song.link connection error: {last_error}", file=sys.stderr
                    )
                ...
                if 400 <= e.response.status_code < 500:
                    print(
                        f"⚠️ song.link client error ({e.response.status_code}): {e}",
                        file=sys.stderr,
                    )
                    return None
        ...
        # All retries failed
        print(
            f"❌ song.link lookup failed after {self.max_retries} attempts: {last_error}",
            file=sys.stderr,
        )
        return None
